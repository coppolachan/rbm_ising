...
############# W parameters
Wnpy = rbm.W.data.numpy()
WtWnpy = np.matmul(Wnpy.T,Wnpy)
# Min and max
Wmin = Wnpy.min()
Wmax = Wnpy.max()
WtWmin = WtWnpy.min()
WtWmax = WtWnpy.max()
Limit = max(abs(Wmin), abs(Wmax))
print ("Wmin = ", Wmin, " Wmax = ", Wmax)
print ("WtWmin = ", WtWmin, " WtWmax = ", WtWmax)

fig,axes = plt.subplots()
coloraxes = axes.imshow(WtWnpy, cmap=cm.viridis)  ## PuBu
axes.set_title("WtW")
cbar = fig.colorbar(coloraxes)
plt.show()
fig.savefig("WtW.png", dpi=300)

fig,axes = plt.subplots(parameters['hidden_layers'],parameters['hidden_layers'])

for i,ax in enumerate(axes.ravel()):
    im = ax.imshow(Wnpy[i].reshape(image_size,image_size), cmap=cm.RdBu, vmin=-Limit, vmax=Limit)
    ax.axis('off')
...
plt.show()
fig.savefig("W.png", dpi=300)

################ Biases
Biases = np.vstack([rbm.v_bias.data.numpy(), rbm.h_bias.data.numpy()])

fig,axes = plt.subplots()
coloraxes = axes.imshow(Biases, cmap=cm.PuBu)
axes.set_title("Biases")
cbar = fig.colorbar(coloraxes)
plt.show()
fig.savefig("Biases.png", dpi=300)
